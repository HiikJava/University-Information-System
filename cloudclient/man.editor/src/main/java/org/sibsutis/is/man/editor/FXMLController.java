...
public class FXMLController implements Initializable
{
    ...
    @FXML
    private Button Remove_Person_BT;
    ...
    private void Configure_Remove_Person_BT()
    {
        Remove_Person_BT.setOnAction(
                (ActionEvent event) ->
        {   // Обработка нажатия клавиши
            log.log(Level.INFO, "[ManEditor] Нажата кнопка удаления студента {Удалить}");
            removeMan();
        }
        );
    }
    ...
    private boolean removeMan()
    {
        boolean result = false;       
        if (ManManager != null)
        {
            Man man = new Man(); 
            log.log(Level.INFO, "[ManEditor] Определение выбраной строки - " + ManTableView.getSelectionModel().getSelectedIndex());
            //man.Row = (ManTableView.getSelectionModel().getSelectedIndex());
            int row = ManTableView.getSelectionModel().getSelectedIndex();
            log.log(Level.INFO, "[ManEditor] Определение строики и отправка завершена");
            log.log(Level.INFO, "[ManEditor] Попытка удаления экземпляра класса {Man}, выполнение {removeMan}...");
            ManTableView.getItems().remove(row);//remove(row);
            log.log(Level.INFO, "[ManEditor] Выполнение операции {removeMan} завершено");
        }
        return result;
    }
